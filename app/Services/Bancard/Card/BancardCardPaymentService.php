...
class BancardCardPaymentService
{
    ...
    public function startAppointmentPayment(array $data): array
    {
        return DB::transaction(function () use ($data) {
            /** @var AppointmentModel $appointment */
            $appointment = AppointmentModel::query()->findOrFail($data['appointment_id']);
            ...
            $description = $data['description'] ?? ('Appointment #' . $appointment->id);
            $identification = $data['identification'] ?? null;

            $payment = new PaymentModel();
            $payment->id_user = $userId;
            $payment->id_appointment = $appointment->id;
            $payment->id_lab_booking = null;
            ...
            $appointment->id_payment = $payment->id;
            $appointment->payment_status = $this->resolveAppointmentPaymentStatus($paymentTypeId);
            $appointment->payment_reference = null;
            $appointment->payment_provider = self::PROVIDER;
            $appointment->payment_confirmed_at = $this->isAutoPaidPaymentType($paymentTypeId) ? now() : null;
            $appointment->save();

            DB::table('appointment_invoice')
            ...
            $appointment->payment_reference = $payment->provider_reference;
            $appointment->payment_provider = self::PROVIDER;
            $appointment->payment_confirmed_at = null;
            $appointment->save();
    ...
    public function confirmAppointmentPayment(PaymentModel $payment, array $confirmationResponse): PaymentModel
    {
        return DB::transaction(function () use ($payment, $confirmationResponse) {
            ...
            if ($payment->id_appointment) {
                $this->syncAppointmentBillingStatus(
                    (int) $payment->id_appointment,
                    $success,
                    $payment
                );
            }
    ...
    public function cancelAndRollbackAppointmentPayment(PaymentModel $payment, array $rollbackResponse): PaymentModel
    {
        return DB::transaction(function () use ($payment, $rollbackResponse) {
            ...
            if ($payment->id_appointment) {
                $this->syncAppointmentBillingAsUnpaid(
                    (int) $payment->id_appointment,
                    $payment
                );
            }
    ...
    public function markWebhookForAppointment(PaymentModel $payment, array $payload): PaymentModel
    {
    ...
    protected function syncAppointmentBillingStatus(
        int $appointmentId,
        bool $success,
    ...
    protected function syncAppointmentBillingAsUnpaid(
        int $appointmentId,
        PaymentModel $payment
    ): void {
        AppointmentModel::query()
    ...
    protected function resolveAppointmentPaymentStatus(int $idPaymentType): string
    {
